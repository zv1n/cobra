...
bool
cobraClipList::setName(QString dbName)
{
  m_sDBName = dbName;
...
cobraClipList::totalSize() const
{
  ...
  if (!query.exec("SELECT SUM(size) FROM cobraClips;")) {
      debug(LOW, "Failed to exec..\n");
      return 0;
    }
...
cobraClipList::updateClip(cobraClip& clip)
{
  ...
  if (!result || !query.next()) {
      debug(ERROR(CRITICAL), "Failed to update specified clip.\n");
      return false;
    }
...
bool
cobraClipList::removeClipByHash(QString hash)
{
  QString deleteString = "DELETE FROM cobraClips WHERE hash=" % hash % ";";
  return sqlQuery(deleteString);
}
...
cobraClipList::addClip(cobraClip& clip)
{
  ...
  if (clip.getHash().isEmpty()) {
      debug(ERROR(CRITICAL), "Failed to populate clip hash!!\n");
      return false;
    }

  if (containsHash(clip.getHash())) {
      debug(ERROR(CRITICAL), "File already exists in the list!\n");
      return false;
    }

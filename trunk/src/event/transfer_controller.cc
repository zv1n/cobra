...
void
cobraTransferStatistics::updatePath(const QByteArray& uid, QString path)
{
...
void
cobraTransferStatistics::updateSize(const QByteArray& uid, unsigned long size)
{
    QWriteLocker lock(&m_rwlStatLock);
...
void
cobraTransferStatistics::updateCurrent(const QByteArray& uid, unsigned long current)
{
    QWriteLocker lock(&m_rwlStatLock);
...
void
cobraTransferStatistics::updateTime(const QByteArray& uid, unsigned long time)
{
    QWriteLocker lock(&m_rwlStatLock);
...
bool
cobraTransferFile::is(const QByteArray& id) const
{
    return (id == m_baExpectedHash);
}
...
void
cobraTransferFile::setExpectedHash(const QByteArray& hash)
{
...
cobraTransferFile*
cobraTransferController::getFile(const QByteArray& hash)
{
    for (int x=0; x<m_vcftTransfers.count(); x++) {
        ...
        debug(LOW, "Hash: %s\n", hash.data());
        debug(LOW, "Hash: %s\n", m_vcftTransfers[x]->expectedHash().data());
        if (m_vcftTransfers[x]->is(hash))
...
cobraTransferController::transferTrigger()
{
    ...
    if (!file->isPendingCompletion()) {
        ...
        debug(ERROR(CRITICAL), "Pending Completion!\n");
    }
...
cobraTransferController::interceptEvent(cobraTransferEvent *event)
{
    ...
    cobraTransferFile* file = getFile(event->hash());
    if (!file)
        return false;
...
{
    if (!file || file->expectedHash().isEmpty())
        return false;
...
cobraTransferController::removeTransfer(cobraTransferFile* file)
{
    ...
    for (int x=0; x<m_vcftTransfers.count(); x++) {
        ...
        if (!m_vcftTransfers[x]->is(file->expectedHash()))
            continue;
...
{
    if (m_vcftPending.contains(file->expectedHash()))
        return false;

    m_vcftPending[file->expectedHash()] = file;
    return true;
}

cobraTransferFile*
cobraTransferController::getPendingTransfer(const QByteArray& hash)
{
    cobraTransferFile* file = m_vcftPending[hash];

    if (!file)
        return NULL;

    if (file->is(hash)) {
        m_vcftPending[hash] = NULL;
...
QMap<QByteArray, cobraTransferFile*> cobraTransferController::m_vcftPending;

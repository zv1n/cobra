...
int cobraClipUpdateEvent::serialize(QDataStream& connection)
{
    ...
    if (m_iCommand == cobraClipUpdateEvent::RequestSync ||
        m_iCommand == cobraClipUpdateEvent::Reject)
        return size;
...
cobraClipUpdateEventHandler::handleEvent(cobraNetEvent* event)
{
    ...
    case cobraClipUpdateEvent::Reject:
        debug(ERROR(CRITICAL), "Rejected event.\n");
        QMessageBox::warning(NULL, "Rejected Request", "Server rejected request (Are you a guest?)");
        break;
...
cobraClipUpdateEventHandler::handleServerEvent(cobraNetEvent* event)
{
    ...
    if (cobraNetHandler::instance()->getIdAuthorization(event->source()) != ParticipantAuth) {
        debug(CRITICAL, "Guest request file sync!\n");
        cobraClipUpdateEvent* revent = new cobraClipUpdateEvent;
        revent->setDestination(event->source());
        revent->setSource(event->destination());
        revent->setCommand(cobraClipUpdateEvent::Reject);
...
cobraClipUpdateEventHandler::handleSyncRequest(cobraClipUpdateEvent* inevent, cobraClipList* clist)
{
    ...
    debug(HIGH, "Clips to Synchronize: %d\n", list.size());

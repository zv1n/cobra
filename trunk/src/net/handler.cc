...
cobraNetHandler::sendEvent(cobraNetEvent *event)
{
    ...
    QReadLocker locker(&m_cidLock);
    if (!m_cIds.contains(dest)) {
        debug(CRITICAL, "Event: Unknown Outbound Destination: %d\n", dest);
        return false;
    }
...
cobraNetHandler::event(QEvent* event)
{
    ...
    if (cevent->isRequest() && !isServing() && cevent->destination() != cobraMyId) {
...
cobraNetHandler::getEvent(int type)
{
    ...
    if (!handler) {
        debug(ERROR(CRITICAL), "Failed to get event handler: %d\n", type);
        return NULL;
    }
...
void
cobraNetHandler::authorizeConnection(int dx)
{
    emit newConnection(dx);
}
